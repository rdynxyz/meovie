        ...
        success: function(result){
            if(result.Response == "True"){
                ...
                $.each(movies, function(i, data){
                    $('#movie-list').append(`
                    <d iv class="col-span-1 bg-gray-800 border rounded-lg shadow-md border-gray-700">
                        <div class="h-3/4 relative overflow-hidden">
                            <img class="rounded-t-lg w-full " src="${data.Poster}"/>
                        </div>
                        <div class="px-5 pt-5">
                            <h5 class="mb-2 text-xl font-bold tracking-tight text-white">${data.Title}</h5>
                            <p class="mb-3 font-normal text-gray-400">${data.Year}</p>
